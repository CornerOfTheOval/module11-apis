...
getMovieInfo <- function(movie.name){
  params <- list("api-key" = api.key, "query" = movie.name)
  raw <- GET("https://api.nytimes.com/svc/movies/v2/reviews/search.json", query = params )
  print(content(raw, as = "text"))
}
getMovieInfo("american psycho")
